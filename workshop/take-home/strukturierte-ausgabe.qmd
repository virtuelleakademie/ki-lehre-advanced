...
**API mit Schema-Definition (hard constraint).** Du übergibst dem Modell ein formales Schema, das die zugelassene Form der Antwort beschreibt. Das LLM wird über eine spezielle API-Funktion aufgerufen (Anthropic *tool use*, OpenAI *structured outputs*), die intern eine Antwort generiert und gegen das Schema validiert. Wenn die Antwort nicht passt, wird sie verworfen oder zur Korrektur zurückgeschickt. Was bei dir ankommt, *muss* dem Schema entsprechen. Das ist die Form, mit der das Block-3-Werkzeug arbeitet.
...
- [Anthropic: Tool Use](https://docs.anthropic.com/en/docs/build-with-claude/tool-use){target="_blank"} (englisch). Die offizielle Anleitung zum Schema-basierten Output bei Claude.
...
- [Pydantic Documentation](https://docs.pydantic.dev/latest/){target="_blank"} (englisch). Die in beiden Workshop-Iterationen verwendete Python-Bibliothek für Schemata.

Die Anthropic Console und das OpenAI Playground bieten beide UI-Oberflächen, in denen man einen Tool-Use-Aufruf zusammenstellen kann, ohne eine eigene Codebasis aufzubauen. Vollständig ohne Code geht es allerdings nicht: das Schema selbst muss in einer von der API verstandenen Form vorliegen.
